...
class AirflowLocalPipelineProcessor(RuntimePipelineProcessor):
    _type = RuntimeProcessorType.APACHE_AIRFLOW_LOCAL
    _name = "airflow_local"
...
be fully qualified (i.e., prefixed with their package names).
    ...
    def process(self, pipeline: Pipeline) -> "AirflowLocalPipelineProcessorResponse":
        """
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            return AirflowLocalPipelineProcessorResponse(
                ...
                if import_stmt:
                    ...
                    if len(component.name.split(".")) > 1:  # We (presumably) have a package prefix
                        ...
                        raise ValueError(
                            ...
                            f"AirflowLocalPipelineProcessor.available_airflow_operators configuration."
                        )
    ...
    def create_pipeline_file(
        ...
        if pipeline_export_format == "json":
            ...
            loader = PackageLoader("elyra", "templates/airflow_local")
            template_env = Environment(loader=loader)

            template_env.filters["regex_replace"] = lambda x: AirflowLocalPipelineProcessor.scrub_invalid_characters(x)
            template = template_env.get_template("airflow_local_template.jinja2")
    ...
    def _scrub_invalid_characters_from_list(self, operation_list: List[Operation]) -> List[Operation]:
        for operation in operation_list:
            operation.name = AirflowLocalPipelineProcessor.scrub_invalid_characters(operation.name)
...
class AirflowLocalPipelineProcessorResponse(RuntimePipelineProcessorResponse):
    _type = RuntimeProcessorType.APACHE_AIRFLOW_LOCAL
    _name = "airflow_local"
