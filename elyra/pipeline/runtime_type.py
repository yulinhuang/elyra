...
class RuntimeProcessorType(Enum):
    ...
    APACHE_AIRFLOW_LOCAL = "Apache Airflow local"
    ARGO = "Argo"
    ######################################
...
class RuntimeTypeResources(object):
    ...
    def get_instance_by_type(
        ...
        if runtime_type == RuntimeProcessorType.APACHE_AIRFLOW_LOCAL:
            return ApacheAirflowLocalResources(runtime_enabled=runtime_enabled)
...
class ApacheAirflowLocalResources(RuntimeTypeResources):
    """Holds static information relative to Apache Airflow local processors"""

    type = RuntimeProcessorType.APACHE_AIRFLOW_LOCAL
    icon_endpoint = "static/elyra/docker-svgrepo-com.svg"
    export_file_types = [{"id": "py", "display_name": "Airflow DSL Python code generated by airflow local processor"}]
